...
Let $\mathcal{E}$ be a semipositive locally free sheaf on $X$ of rank $m$.
Set $\mathcal{F} = \mathrm{Sym}^d(\mathcal{E})$ for some $d \geq 1$ and set
$n = \binom{m + d}{d}$ be the rank.
...
\todo{u is quasifinite on open subset of orbit P}

The map $u$ need not be a morphism since the image of $\overline{U}$ need not be locally free.
To remedy this we blow up the ideal sheaf of the image of $\overline{U}$\todo{explain better}, obtaining a new scheme $P'$, together with a birational morphism $g:P'\to P$.
