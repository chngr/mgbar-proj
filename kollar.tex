...
Let $Y$ be a normal projective variety over a field $k$.
Let $V$ be a semipositive vector bundle on $Y$ with structure group $G$.
Set $W = \mathrm{Sym}^d(V)$ for some $d \geq 1$ and let $n$ be the rank.
Let $Q$ be a quotient vector bundle of $W$ of rank $k$.
Assume that the map
$$
u : Y \to [\mathrm{Gr}(n,k)/G]
$$
is quasi-finite on a nonempty open subset of $Y$.
Then $(\det(Q))^{\dim(Y)} > 0$.
\end{lemma}
