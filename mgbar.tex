...
\section{Nefness for locally free sheaves}
...
We say that $V$ is \textit{nef} if for every map from a proper and
...
\begin{lemma}
Quotients and extensions of nef vector bundles are nef.
\end{lemma}
...
Let $V$ be a nef vector bundle on a scheme $X$.
For any morphism $g : Y \to X$, the pullback $f^*V$ is nef vector bundle
on $Y$.
\end{lemma}
...
Then, as $f^*(g^*V)=(g\circ f)^*V$ and $V$ is nef, any quotient
...
Then, $\mathcal{V}$ is nef if and only if
...
Then, $\deg \mathcal{O}_{\mathbb{P}L}(1)=\deg L\ge0$, so $V$ is nef.
...
By Lemma \ref{pullback-of-semipos}, $\pi^*V$ is nef, so
