...
Then, by the semismoothness condition\todo{I think.}, $D$ must be the
intersection of two
irreducible components in $S$, and blowing up $D$ gives a map $b :
\widetilde{S} \to S$ that separates the two irreducible components.
There is a natural short exact sequence\todo{Why?}
$$0 \longrightarrow b_*(\omega_{\widetilde{S}/C}^{\otimes m})
\longrightarrow \omega_{S/C}^{\otimes m} \longrightarrow
\mathcal{K} \longrightarrow 0,$$
where the support of $\mathcal{K}$ is in $D$. Pushing forward, we have a
semipositive bundle $\mathcal{K}$, hence $f_*(\omega_{S/C}^{\otimes m})$ is the
extension of semipositive bundles, hence semipositive.
\par We now claim that we may assume that every double curve $D$ in $S$ is a
section of $f : S \to C$.
If it is not, then we can base change by 
\end{proof}
