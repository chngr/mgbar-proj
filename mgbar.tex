...
genus $g \geq 2$.
Let $b : S' \to S$ is a resolution of the isolated singularities of $S$
and let $f' : S' \to C$ be the composition $f' = f \circ b$.
If $f'_*(\omega_{S'/C}^{\otimes m})$ is semipositive for $m \geq 2$, then
...
\begin{lemma}[cf.\ {\cite[Corollary 4.6]{Ko90}}]\label{intersection_with_section}
Suppose we are in Situation \label{kollar_prop_4.7} where $g\ge2$. Then, $\omega_{S/C}\cdot C_t\ge0$ for any $t$.
\end{lemma}

\begin{proof}
Let $i:C_t\hookrightarrow S$ be the inclusion map, and $g=f\circ i$.
...
Indeed, by Cohomology and Base Change and the fact that $f$ has relative dimension 1, it suffices to note that $H^{1}(S_x,\omega_{S_x/\kappa(x)}^{\otimes m}(D_{j-1}\cdot S_x))=0$ for each fiber $S_x$ of $f$, which is clear by Serre Duality and degree considerations.
On the other hand, when $D_{j-1}=0$, Grauert's Theorem shows that $R^{1}f_{*}\omega_{S/C}^{\otimes m}(D_{j-1}))$ is an invertible sheaf.
...
Assume the claim is proven for $D_{j-1}=\sum a_iC_i$; we will prove it for $D_{j}=D_{j-1}+C_t$.
By Lemma \ref{intersection_with_section}, $\omega_{S/C}\cdot C_t\ge0$.
