...
\chapter[Projectivity of Moduli of Curves]{Projectivity of Moduli of Curves}

\contributor{Raymond Cheng,
  \affiliation{Department of Mathematics, Columbia University, 2990~Broadway, New York, NY~10027, United States}}

\contributor{Carl Lian,
  \affiliation{Institut für Mathematik, Humboldt-Universität zu Berlin Rudower Chaussee 25, Raum 1.412 12489 Berlin}}

\contributor{Takumi Murayama,
  \affiliation{Department of Mathematics, Princeton University, Princeton, NJ 08544-1000, USA}}

\arabicfootnotes

\chapterabstract{\ldots}

\section{Introduction}
\label{section-introduction}

\noindent
In this article, we exposit a proof, due to Koll\'ar in~\cite{ko90}, of the fact
that the moduli stack $\overline{\mathcal{M}}_g$ of stable curves of genus
$g \geq 2$ admits a coarse moduli space $\overline{M}_g$ which is projective
over $\mathbf{Z}$.
In particular, this means that the coarse moduli space $\overline{M}_g$, which
is \emph{a priori} but an algebraic space, is actually a projective scheme
over $\mathbf{Z}$.
Together with the work of Deligne--Mumford~\cite{dm}, the moduli space
$\overline{M}_g$ is actually an irreducible smooth projective scheme over
$\mathbf{Z}$.

\medskip
\noindent
Koll\'ar's method differs from other existing proofs of projectivity of
$\overline{M}_g$ in two main ways: First, the technique is independent of the
powerful, though bulky, methods of Geometric Invariant Theory, on which
the proofs of~\cite{mumford_stability, gieseker_lectures, cornalba_projectivity}
are reliant on. Second, Koll\'ar's criterion does not require one to directly
check that a line bundle on the moduli space is ample, in contrast to the
approach of Knudsen--Mumford~\cite{km-projectivity-I, knudsen-II, knudsen-III};
rather, one only needs to show that some vector bundle on the moduli space
is nef.
Since nefness is a condition that only depends on curves in the space,
concretely, this condition can be checked in moduli situations by considering
families over a $1$-dimensional base.
Thus, the positivity input in this proof amounts to studying positivity of
the pushfoward of relative canonical sheaves on families of stable curves
over a $1$-dimensional base.\todo{Write a legitimate introduction. What is this
...
% REFERENCE: \cite[Example 1.4.4(i)]{lazI}
...
% REFERENCE: \cite[Example 1.4.4(ii)]{lazI}
...
\section{Interesting mathematics}
...
% bibliography comes here; either use a BibTeX file or \bibitem's
% please use MathSciNet identifiers whenever available; Pieter will ensure consistency
% for citing the Stacks project you can use the key stacks-project
