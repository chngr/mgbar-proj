...
$$\mathcal{L}\rvert_C = (f^*\mathcal{O}_{\mathbf{P}^n}(1))\rvert_C \simeq
\pi^*\mathcal{O}_{\Spec(k(y))} = \mathcal{O}_C,$$
...
are proper, we moreover have that $f$ is finite by 02OG. Finally, the pullback
of an ample invertible sheaf via a finite morphism is ample by applying Lemma
\ref{lemma-surjective-finite-morphism-ample} to the finite surjective morphism
$X \to f(X)$, hence $\mathcal{O}_X(mD)$ is ample for $m \gg 0$, hence
$\mathcal{O}_X(D)$ is ample by Lemma \ref{lemma-ample-power-ample}.
\end{proof}
